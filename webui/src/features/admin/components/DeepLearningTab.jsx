...
import { Brain, FileText, Activity, Database, GitMerge, FileImage, Play, CheckCircle2, XCircle, Clock } from 'lucide-react';
...
const DeepLearningTab = () => {
  ...
  const handleSubmitJob = async () => {
    ...
    try {
      ...
      setShowModal(false);
      setSelectedDoc(null);
      fetchJobs(); // Trigger immediate update
    } catch (e) {
  ...
  const renderStatusBadge = (status) => {
    switch (status) {
      ...
      default: return <span className="flex items-center gap-1 text-slate-400 bg-slate-500/10 px-2 py-1 rounded text-xs"><Clock className="w-3 h-3" /> Pending</span>;
    }
  };
    ...
    <div className="flex flex-col gap-6 text-slate-200 animate-in fade-in duration-500">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-400 text-sm mt-1">
              Pusat orkestrasi Deep Learning. Tarik dokumen dari Portal Pindad (MySQL) dan injeksikan ke dalam struktur otak CAKRA (RAG DB).
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="xl:col-span-2 bg-slate-900/50 border border-slate-800 rounded-2xl overflow-hidden flex flex-col h-[600px]">
          <div className="p-4 border-b border-slate-800 bg-slate-900 flex justify-between items-center">
            <h3 className="font-semibold text-slate-200 flex items-center gap-2">
              <Database className="w-4 h-4 text-blue-400" /> MySQL Source: Tabel Berita
            </h3>
      ...
      {showModal && selectedDoc && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-2xl overflow-hidden animate-in zoom-in-95 duration-200 shadow-2xl">
            <div className="p-6 border-b border-slate-800">
              <h3 className="text-xl font-bold text-slate-200">Konfigurasi Pelatihan AI</h3>
              <p className="text-sm text-slate-400 mt-1">Dokumen: <span className="text-white font-medium">{selectedDoc.judul}</span></p>
            </div>
            
            <div className="p-6">
              <label className="text-sm font-medium text-slate-300 mb-4 block">Pilih Metode Ingesti (Deep Learning Architecture):</label>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {TRAINING_METHODS.map(method => (
                  ...
                  >
                    <div className={`p-2 rounded-lg ${trainingMethod === method.id ? 'bg-purple-500/20 text-purple-400' : 'bg-slate-800 text-slate-400'}`}>
                      {React.cloneElement(method.icon, { className: 'w-5 h-5' })}
                    </div>
                    <div>
                      <h4 className={`font-semibold ${trainingMethod === method.id ? 'text-purple-300' : 'text-slate-300'}`}>{method.title}</h4>
            ...
            <div className="p-4 border-t border-slate-800 bg-slate-950 flex justify-end gap-3">
              <button 
                ...
                className="px-5 py-2.5 text-slate-300 hover:text-white font-medium rounded-lg hover:bg-slate-800 transition-colors"
              >
                Batal
              </button>
              <button 
                ...
                className="px-5 py-2.5 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white font-medium rounded-lg transition-all flex items-center gap-2 disabled:opacity-50"
              >
