...
const copyToClipboard = async (text, showNotification) => {
  ...
  // Metode 2: Fallback (Textarea hack)
  try {
    const textArea = document.createElement("textarea");
    textArea.value = text;
    
    // Pastikan textarea tidak terlihat tapi ada di DOM
          ...
          return (
            <hr
              ...
              className={`my-4 border-t ${
                isAI ? "border-gray-200" : "border-white/20"
              }`}
            />
          );
        }
        ...
        if (isHTML) {
          ...
          return (
            <div className="flex" key={lineIndex}>
              <span className={`font-bold min-w-[1.2rem] ${accentColor}`}>
                {number}.
              </span>
              <div
                className="ml-2"
...
const renderStyledText = (text, isAI) => {
  ...
  processedText = processedText.replace(
    /^#### (.*$)/gm,
    `<h4 class="text-base font-bold mt-3 mb-1 ${hColor}">$1</h4>`
  );
  processedText = processedText.replace(
    /^### (.*$)/gm,
    `<h3 class="text-lg font-bold mt-4 mb-1 ${hColor}">$1</h3>`
  );
  processedText = processedText.replace(
    /^## (.*$)/gm,
    `<h2 class="text-xl font-bold mt-5 mb-2 ${hColor}">$1</h2>`
  );
  processedText = processedText.replace(
    /^# (.*$)/gm,
    `<h1 class="text-2xl font-extrabold mt-6 mb-3 ${hColor}">$1</h1>`
  );

  // 2. Blockquotes
  processedText = processedText.replace(
    /^> (.*$)/gm,
    `<blockquote class="border-l-4 ${bColor} pl-4 italic my-2">$1</blockquote>`
  );

  // 3. Links [text](url)
  processedText = processedText.replace(
    /\[([^\]]+)\]\(([^)]+)\)/g,
    `<a href="$2" target="_blank" rel="noopener noreferrer" class="${linkColor}">$1</a>`
  );
  ...
  processedText = processedText.replace(
    /`([^`]+)`/g,
    `<code class="${codeClass} px-1 rounded font-mono text-xs break-all">$1</code>`
  );
  ...
  processedText = processedText.replace(
    /^(--[-]+|\*\*[\*]+)$/gm,
    `<hr class="my-4 border-t ${hrColor}" />`
  );
            ...
            <div className="space-y-6">
              {messages.map((msg) => (
                <div
                  key={msg.id}
                  className={`flex ${
                    msg.sender === "user" ? "justify-end pl-12" : "justify-start pr-12"
                  }`}
                >
                  <div
                    className={`${
