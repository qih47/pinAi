...
def chunk_se(text: str) -> List[Dict[str, Any]]:
    ...
    while i < len(lines):
        ...
        # Deteksi butir utama: "1.", "2.", "10.", dll. (hanya jika dalam konteks struktur hukum)
        elif (re.match(r'^\s*\d+\.\s*$', stripped) or re.match(r'^\s*\d+\.\s+\S', stripped)) and in_structured_section:
            # Simpan section sebelumnya jika ada
            if current_section:
                # Gabungkan sub-items ke main section
                full_content = current_section['content']
