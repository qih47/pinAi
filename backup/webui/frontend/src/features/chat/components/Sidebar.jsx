...
const Sidebar = ({
  ...
  const togglePinChat = async (e, sessionUuid) => {
    e.stopPropagation();
    try {
      const res = await chatApi.pinChat(sessionUuid);
  ...
  const handleRename = async (sessionUuid) => {
    ...
    try {
      const res = await chatApi.renameChat(sessionUuid, tempTitle);
      const result = await res.json();
      if (result.status === "success") {
        setChatHistory((prev) =>
          prev.map((chat) =>
  ...
  const executeDelete = async () => {
    ...
    try {
      ...
      const res = await chatApi.deleteChat(sessionToDelete);
      const result = await res.json();
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      if (userData?.username) {
        try {
          const res = await chatApi.fetchChatHistory(userData.username);
          const result = await res.json();
          if (result.status === "success") {
