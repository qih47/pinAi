...
class ModeFocus:
    ...
    Membaca dokumen PDF (teks/scan) menggunakan Universal Fast OCR & Two-Stage Context-Aware Reranking.
    """
    ...
    async def execute(
        ...
        yield format_sse(status="⚙️ Memindai & memproses halaman dokumen...", event_type=SSEEventType.STATUS)
        await asyncio.sleep(0.05)
        ...
        yield format_sse(status=f"🔍 Memindai seluruh {total_pages} halaman dokumen...", event_type=SSEEventType.STATUS)
        await asyncio.sleep(0.05)
        ...
        yield format_sse(status=f"📌 Ditemukan Halaman {halaman_str} (Konteks Terstruktur Utuh)!", event_type=SSEEventType.STATUS)
        await asyncio.sleep(0.2)
        
        yield format_sse(status=f"👁️ Menelaah bagan dan teks pada Halaman {halaman_str}...", event_type=SSEEventType.STATUS)
