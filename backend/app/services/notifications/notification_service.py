...
class NotificationBroker:
    ...
    def __init__(self):
        ...
        logger.info("[NOTIFICATION_BROKER_INIT] Notification broker initialized.")
    ...
    async def subscribe(self, npp: str) -> asyncio.Queue:
        ...
        async with self._lock:
            ...
            logger.info(f"[NOTIF_SUBSCRIBE_SUCCESS] NPP {npp} subscribed (total subscribers: {len(self.subscribers[npp])})")
            return queue
    ...
    async def unsubscribe(self, npp: str, queue: asyncio.Queue):
        ...
        async with self._lock:
            if npp in self.subscribers and queue in self.subscribers[npp]:
                ...
                logger.info(f"[NOTIF_UNSUBSCRIBE_SUCCESS] NPP {npp} unsubscribed")
    ...
    async def publish(self, notification: Notification):
        ...
        async with self._lock:
            ...
            if recipient_npp not in self.subscribers:
                logger.debug(f"[NOTIF_PUBLISH_NO_SUBSCRIBER] No active subscribers for NPP: {recipient_npp}")
                return
            
            queues = list(self.subscribers[recipient_npp])
            logger.info(
                f"[NOTIF_PUBLISH_SUCCESS] Sending {notification.type.value} to {len(queues)} subscribers (NPP: {recipient_npp})"
            )
        ...
        for queue in queues:
            try:
                ...
                logger.warning(f"[NOTIF_PUBLISH_QUEUE_FULL] Queue full for subscriber (NPP: {recipient_npp})")
    ...
    async def publish_to_all(self, notification: Notification):
        ...
        async with self._lock:
            ...
            logger.info(f"[NOTIF_BROADCAST_SUCCESS] Sending {notification.type.value} to {len(all_queues)} total subscribers")
        
        for queue in all_queues:
            try:
                queue.put_nowait(notification.to_json())
            except asyncio.QueueFull:
                logger.warning("[NOTIF_BROADCAST_QUEUE_FULL] Queue full for subscriber in broadcast")
