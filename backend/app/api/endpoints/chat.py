...
# ==============================================================================
# 🚀 SECTION 2: ENDPOINT UTAMA CORE THREE-ENGINE AGENTIC CHAT
# ==============================================================================
...
async def chat_stream_endpoint(
    ...
    # 1. AUTO-SAVE DATA OBROLAN USER KE DALAM RAGDB
    if payload.session_uuid and current_user_npp:
        print(
            f"💾 [CHAT API] Auto-save: Menyimpan pesan USER & Hasil Analisis Kognitif untuk sesi {payload.session_uuid[:8]}..."
        )
    ...
    # 2. Rekonstruksi array payload pesan untuk LLM target
    formatted_messages = [
    ...
    if detected_intent in ["RAG", "ANALYTICS"]:
        print(
            f"🔀 [ROUTER DECISION] Intent Kritis '{detected_intent}' Terdeteksi! Membelokkan Traffic ke Slot 2 Reasoning Engine ({settings.MODEL_REASONING})..."
        )

        # 🚀 EMBEDDING & SEARCH INJECTION (Advanced RAG Power)
        # Panggil fungsi perakit konteks parent-child berkekuatan penuh
        ...
        if rag_context:
            print(
                "📎 [CONTEXT INJECTED] Dokumen Pindad sukses disuntikkan ke dalam ingatan jangka pendek DeepSeek R1!"
            )
            system_content += (
            ...
            print(
                "ℹ️  [CONTEXT EMPTY] Tidak ada dokumen pendukung yang lolos threshold. DeepSeek beroperasi dengan mode penalaran murni."
            )
        ...
        # Stream dialirkan menggunakan orkestrator DeepSeek-R1 yang menangani token <think>
    ...
    # JALUR B: Kueri Bersifat Chit-Chat / Operasional Umum Ringan (SLOT 3 - GEMMA 4)
        ...
        print(
            f"🤖 [ROUTER DECISION] Intent '{detected_intent}'. Menetap di Slot 3 Persona Engine ({settings.MODEL_PERSONA})..."
        )

        # 🧠 SUNTIKAN MEMORI: Tarik memori masa lalu pegawai dari RAGDB
        npp_query = current_user_npp if current_user_npp else "GUEST"
        past_memory_context = await memory_service.get_employee_long_term_memory(
            npp_query
        )
        ...
        # Gabungkan prompt dasar dengan ingatan masa lalu jika ada
        if past_memory_context:
            base_prompt += past_memory_context

        if detected_sentiment == "FRUSTRATED":
            base_prompt += " NOTE: Karyawan sedang mengalami kendala/frustrasi teknis. Redam situasi dengan empati taktis..."
        print(
            f"🤖 [ROUTER DECISION] Intent '{detected_intent}'. Menetap di Slot 3 Persona Engine ({settings.MODEL_PERSONA})..."
        )
        ...
        # Stream dialirkan menggunakan engine standard chat session milik Gemma
        return StreamingResponse(
            stream_ollama_chat(
                model_name=settings.MODEL_PERSONA,
...
# ==============================================================================
# 🧪 SECTION 3: ENDPOINT TESTING KILAT UTALITAS AGENT
# ==============================================================================


@router.post("/test-router", summary="⚡ TEST KILAT: Uji Kognitif Slot 1 Router Engine")
async def test_router_endpoint(payload: TestRouterRequestSchema):
    """Endpoint utilitas untuk menguji akurasi klasifikasi JSON mentah dari Qwen 2.5"""
    print(f'\n🧠 [TEST ROUTER] Menerima teks uji coba: "{payload.text}"')
    analysis = await router_engine.analyze_user_query(payload.text)
